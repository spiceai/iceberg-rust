...
impl<'a> SnapshotProducer<'a> {
    ...
    pub(crate) fn with_delete_files(mut self, delete_files: Vec<DataFile>) -> Self {
        ...
        for delete_file in &self.added_delete_files {
            match delete_file.content_type() {
                ...
                _ => {
                    return Err(Error::new(
                        ErrorKind::DataInvalid,
                        "Only equality or position delete content types are allowed for delete files",
                    ));
                }
            }
    ...
    async fn write_added_delete_manifest(&mut self) -> Result<ManifestFile> {
        ...
        let snapshot_id = self.snapshot_id;
        let format_version = self.table.metadata().format_version();
        let manifest_entries = added_delete_files.into_iter().map(|data_file| {
            let builder = ManifestEntry::builder()
                .status(crate::spec::ManifestStatus::Added)
                .data_file(data_file);
            if format_version == FormatVersion::V1 {
                // V1 does not support delete files; this should not be reached
                // but handle gracefully by setting snapshot_id
                builder.snapshot_id(snapshot_id).build()
            } else {
                builder.build()
            }
